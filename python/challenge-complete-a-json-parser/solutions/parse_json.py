...
def parse_json(json_str: str) -> Any:
    ...
    def parse_value() -> Any:
        ...
        skip_whitespace()
        if json_str[pos] == '{':
            return parse_object()     # Call parse_object function if the current character is an opening brace
        elif json_str[pos] == '[':
            return parse_array()      # Call parse_array function if the current character is an opening bracket
        elif json_str[pos] == '"':
            return parse_string()     # Call parse_string function if the current character is a double-quote
        elif json_str[pos].isdigit() or json_str[pos] == '-':
            return parse_number()     # Call parse_number function if the current character is a digit or minus sign
        elif json_str[pos] == 't' or json_str[pos] == 'f':
            return parse_boolean()    # Call parse_boolean function if the current character is 't' or 'f'
        elif json_str[pos] == 'n':
            return parse_null()       # Call parse_null function if the current character is 'n'
        else:
    ...
    def parse_object() -> Dict[str, Any]:
        ...
        while True:
            skip_whitespace()
            if json_str[pos] == '}':  # If the current character is a closing brace, return the object
                pos += 1
                return obj
    ...
    def parse_array() -> List[Any]:
        ...
        while True:
            skip_whitespace()
            if json_str[pos] == ']':  # If the current character is a closing bracket, return the array
                pos += 1
                return arr
            val = parse_value()       # Parse the value recursively
    ...
    def parse_string() -> str:
        nonlocal pos
        start = pos + 1
        pos = json_str.find('"', start)  # Find the next double-quote character starting from the current position
        if pos == -1:
    ...
    def parse_number() -> Union[int, float]:
        nonlocal pos
        start = pos
        while pos < len(json_str) and (json_str[pos].isdigit() or json_str[pos] in ['.', 'e', 'E', '+', '-']):
            pos += 1
