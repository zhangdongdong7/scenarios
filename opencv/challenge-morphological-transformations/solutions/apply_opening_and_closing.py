...
def apply_opening_and_closing(image: np.ndarray, kernel_shape: str, kernel_size: int, opening_iterations: int, closing_iterations: int) -> np.ndarray:
    """
    ...
    opened_image = cv2.morphologyEx(image, cv2.MORPH_OPEN, kernel, iterations=opening_iterations)
    closed_image = cv2.morphologyEx(opened_image, cv2.MORPH_CLOSE, kernel, iterations=closing_iterations)
